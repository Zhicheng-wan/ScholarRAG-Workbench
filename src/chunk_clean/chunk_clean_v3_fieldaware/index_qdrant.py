...
def main() -> None:
    ...
    # Simple set of "low-signal" sections to skip entirely
    LOW_VALUE_SECTIONS = {
        "references",
        "reference",
        "bibliography",
    ...
    for batch in batched(record_iter, args.batch_size):
        ...
        for rec in batch:
            ...
            # Optional: skip low-value structural sections
            section = (rec.get("section") or "").strip()
            if section.lower() in LOW_VALUE_SECTIONS:
                continue

            title = (rec.get("title") or "").strip()

            # Build field-aware text: title | section | body
            pieces = [p for p in (title, section, raw_text) if p]
            if not pieces:
                continue

            combined_text = " | ".join(pieces)

            # --- per-chunk id, but paper-level doc_id remains in payload ---
            # Prefer explicit chunk_id, then anchor, then a stable hash of combined_text.
            raw_chunk_id = (
                rec.get("chunk_id")
                or rec.get("anchor")
                or hashlib.md5(combined_text.encode("utf-8")).hexdigest()[:8]
            )
            ...
            if args.store_text:
                payload["text"] = combined_text[: args.max_text_length]
